...
class Watch {
  ...
  /**
   * Starts watch and sets Watch.details
   * @returns {void}
   */
  _startWatch() {
   ...
   * @param {({ name, millis}) => {}} callback Func that will be called with the changed file's details
   */
  onChange(callback) {
   ...
   */
  _subscribe() {
    if (!this.resp || !this.resp.watch || !this.resp.relative_path) {
